...
set out40 [open out40.nam w]

$ns namtrace-all $out40

set saida40 [open saida40.tr w]
$ns trace-all $saida40
...
# 40 Transmissores: (estao na 1a arvore)
for {set i 0} {$i < $TRANS40} {incr i} {
    ...
    $ns connect $udp($i) $null([expr $i/($TRANS40/$RECS)])

    # Definindo as propriedades da conexao
    $udp($i) set fid_ $i
    $udp($i) set packetSize_ 500

	# Agora, ligando uma aplicacao com as 40 maquinas
