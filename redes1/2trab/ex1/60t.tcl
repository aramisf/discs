...
# - variar a qtde de transmissores 60/40/50/60 transmissores - todos os
...
set out60 [open out60.nam w]

$ns namtrace-all $out60

set saida60 [open saida60.tr w]
$ns trace-all $saida60
...
for {set i 0} { $i < [expr $SUBSWITCHES*2] } {incr i} {
	...
	$ns duplex-link $sub_switch($i) $main_switch([expr $i/$SUBSWITCHES]) 10Mb 10ms DropTail
...
# 60 Transmissores: (estao na 1a arvore)
for {set i 0} {$i < $TRANS60} {incr i} {
    ...
    $ns connect $udp($i) $null([expr $i/($TRANS60/$RECS)])

    # Definindo as propriedades da conexao
    $udp($i) set fid_ $i
    $udp($i) set packetSize_ 500

	# Agora, ligando uma aplicacao com as 60 maquinas
